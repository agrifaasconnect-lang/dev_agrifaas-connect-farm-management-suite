...
import type { Task, User, Plot, Comment, InventoryItem } from '../types';
        ...
        <Modal isOpen={!!task} onClose={onClose} title={isEditing ? 'Edit Task' : task.title} size="2xl">
            {isEditing ? (
                ...
                <div className="space-y-6">
                    ...
                    {task.inventoryConsumed && task.inventoryConsumed.length > 0 && (
                        <div className="pt-4 border-t">
                            <h4 className="font-semibold text-sm mb-2 text-gray-700">Inventory Used</h4>
                            <ul className="list-disc list-inside text-sm text-gray-600">
                                {task.inventoryConsumed.map((item, index) => {
                                    const invItem = inventory.find(i => i.id === item.inventoryId);
                                    return (
                                        <li key={index}>
                                            {invItem ? invItem.name : 'Unknown Item'}: {item.quantityUsed} {invItem?.unit}
                                        </li>
                                    );
                                })}
                            </ul>
