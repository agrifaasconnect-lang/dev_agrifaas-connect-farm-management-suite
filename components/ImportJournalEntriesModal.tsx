...
export interface ImportJournalEntriesModalProps {
    isOpen: boolean;
    onClose: () => void;
    accounts: Account[];
    plots: Plot[];
...
export const ImportJournalEntriesModal: React.FC<ImportJournalEntriesModalProps> = ({ isOpen, onClose, accounts, plots, seasons, onImport }) => {
    ...
    const processFile = async (fileToProcess: File) => {
        ...
        try {
            ...
            const accountMap = new Map<string, string>(accounts.map(a => [a.name.toLowerCase(), a.id]));
            const plotMap = new Map<string, string>(plots.map(p => [p.name.toLowerCase(), p.id]));
            const seasonMap = new Map<string, string>(seasons.map(s => [s.name.toLowerCase(), s.id]));
            ...
            for (const key in entryGroups) {
                ...
                for (const row of group) {
                    ...
                    const plotName = String(row['plotname'] || '');
